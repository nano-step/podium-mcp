...
export function findElements(
  ...
  sel: { text?: string; id?: string }
): NativeElement[] {
  const matches: NativeElement[] = [];
...
export interface NativeBackend {
  ...
  inputText(udid: string, text: string): Promise<RunResult>;
  /** True when pressKey(key) has a native mapping (no side effects). */
  canPressKey(key: string): boolean;
  /** null = key has no native mapping (caller falls back to Maestro). */
  pressKey(udid: string, key: string): Promise<RunResult | null>;
  /** Flat accessibility-element list, or null on failure. */
  describeAll(udid: string): Promise<NativeElement[] | null>;
  /** Logical-point screen dimensions, or null when unknown. */
  screenPoints(udid: string): Promise<{ w: number; h: number } | null>;
  /** Native orientation set; null = unsupported value/backend. */
  setOrientation(udid: string, value: string): Promise<RunResult | null>;
  /** Native orientation query → "portrait" | "landscape" | null when unavailable. */
  getOrientation?(udid: string): Promise<string | null>;
}

// ─── mobilecli binary resolution ─────────────────────────────────────────────

/** Map node platform/arch to mobilecli's binary naming. */
function mobilecliBinaryName(): string | null {
  const plat =
...
let cachedMobilecli: string | null | undefined;
 ...
 */
export async function resolveMobilecli(): Promise<string | null> {
  if (cachedMobilecli !== undefined) return cachedMobilecli;
...
const MOBILECLI_BUTTONS: Record<string, string> = {
  home: "HOME",
  lock: "POWER",
  power: "POWER",
  "volume up": "VOLUME_UP",
...
function makeMobilecliBackend(bin: string): NativeBackend {
  return {
    ...
    screenPoints: async (udid) => {
      ...
      if (hit && Date.now() - hit.at < 10_000) return { w: hit.w, h: hit.h };
      const r = await run(bin, ["device", "info", "--device", udid], { timeout: 15_000 });
      if (r.code !== 0) return null;
      try {
        const parsed = JSON.parse(r.stdout) as {
          data?: { device?: { screenSize?: { width?: number; height?: number; scale?: number } } };
        };
    ...
    getOrientation: async (udid) => {
      ...
      if (r.code !== 0) return null;
      try {
        const parsed = JSON.parse(r.stdout) as { data?: { orientation?: string } };
...
function makeIdbBackend(): NativeBackend {
  return {
    name: "idb",
    tap: idbTap,
    swipe: idbSwipe,
